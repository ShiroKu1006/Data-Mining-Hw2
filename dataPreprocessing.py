...
h_amt = df['TransactionAmount'].quantile(0.95)
df['isFraud'] = np.where((df['TransactionAmount'] > h_amt) | ((df['LoginAttempts'] >= 3) | (df['TransactionDuration'] < 20)), 1, 0)
print(f"{df['isFraud'].value_counts()}")
cols_to_drop = [
...
df_train['isFraud'] = y_train.values
